class_name GodotParadiseGridMovement extends Node2D
...
func teleport_to(target_position: Vector2,  valid_position_callback: Callable = _default_valid_position_callback):
	var result = {
		...
		"to": target_position, 
		"direction": target_position if target_position.is_normalized() else target_position.normalized()
	}
...
func _handle_grid_direction(direction: Vector2):
	direction = direction if direction.is_normalized() else direction.normalized()
