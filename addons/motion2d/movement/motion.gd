class_name GodotParadiseMotion extends Node2D
...
func accelerate(direction: Vector2, delta: float =  get_physics_process_delta_time()) -> GodotParadiseMotion:
...
func decelerate(delta: float = get_physics_process_delta_time(), force_stop: bool = false) -> GodotParadiseMotion:
...
func accelerate_to_position(position: Vector2) -> GodotParadiseMotion:
	return accelerate(body.global_position.direction_to(position))


func apply_air_friction_horizontal(friction_factor: float = AIR_FRICTION_HORIZONTAL_FACTOR) -> GodotParadiseMotion:
...
func apply_air_friction_vertical(friction_factor: float = AIR_FRICTION_VERTICAL_FACTOR) -> GodotParadiseMotion:
...
func knockback(direction: Vector2, power: float) -> GodotParadiseMotion:
...
func teleport_to_position(target_position: Vector2, valid_position_callback: Callable = _default_valid_position_callback) -> GodotParadiseMotion:
...
func change_speed_temporary(new_speed: float, time: float = DEFAULT_TEMPORARY_SPEED_TIME) -> GodotParadiseMotion:
...
func dash(target_direction: Vector2 = facing_direction, speed_multiplier: float = DASH_SPEED_MULTIPLIER) -> GodotParadiseMotion:
